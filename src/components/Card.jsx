  ...
  if (selectedProject) {
    return (
      <div className="px-8 pt-0 flex flex-col gap-2 h-full">
        <button
          className="self-start px-3 py-1 bg-gray-700 text-white rounded-lg flex items-center gap-1 cursor-pointer"
        ...
        <div>
          ...
          <div className="flex gap-3 mt-2 flex-wrap">
            {selectedProject.techStack.map((item, i) => (
              <span key={i} className="px-3 py-1 bg-gray-200 text-black rounded-lg">
                {item}
              </span>
            ))}
          </div>
  ...
  return (
    <div className="flex-col flex h-full justify-between">
      <div className="grid grid-cols-2">
        {data?.map((project, index) => (
          <div
            ...
            key={index}
            className="flex flex-col p-4 border border-[var(--subheading-color)] m-4 rounded-lg gap-2 bg-[var(--card-bg)] shadow-xl cursor-pointer hover:scale-105 transition-all duration-300"
          >
      ...
      <div className="flex gap-2 justify-center w-full mb-5">
        ...
        <button
          className={`py-1 px-2 rounded-md border border-slate-400 cursor-pointer ${(page+1)==1?'bg-[var(--text-secondary)] text-white':''}`}
          onClick={() => setPage(0)}
        >
          1
        </button>
        <button
          className={`py-1 px-2 rounded-md border border-slate-400 cursor-pointer  ${(page+1)==2?'bg-[var(--text-secondary)] text-white':''}`}
          onClick={() => setPage(1)}
        >
          2
        </button>
